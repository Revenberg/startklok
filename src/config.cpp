...
void registerConfigRoutes(WebServer &server) {
  ...
  // Telegram setup page
  server.on("/telegram", [&server]() {
    handleTelegram(server);
  });
  
  // Save Telegram config
  server.on("/telegram/save", HTTP_POST, [&server]() {
    ...
    server.send(200, "text/html", "<script>alert('Telegram config saved!'); window.location='/telegram';</script>");
  });
}